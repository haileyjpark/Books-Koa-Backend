...
const createReservation = async (reservationCode, bookInfoId, userId) => {
  const rentals = await rentalRepository.getOneWithBook({ bookInfoId });
  if (!rentals) {
...
const getReservations = async (data, page, limit) => {
  const reservationList = await reservationRepository.getReservations(data, pagination(page, limit));
  if (!reservationList) {
    throw new Error('잘못된 예약 정보를 입력하셨습니다.');
  }
  return reservationList;
};

// 예약 상세 정보 조회
const getOneReservation = async (data) => {
...
const getOldReservations = async (data, page, limit) => {
  const oldReservationList = await deactivatedReservationRepository.getOldReservations(data, pagination(page, limit));
